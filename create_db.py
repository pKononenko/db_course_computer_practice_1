...
def db_insert_batched_data(dbname, username, password, host, port, filename, batched_data_dir_name = "batched_data", table_name = "ZNODATA"):
    conn = None
    batches_files_path_list = sorted(listdir(batched_data_dir_name), key = len)
    column_names_list, _ = get_column_names_types(filename)

    start = time.time()
    while batches_files_path_list:
        try:
            conn = psycopg2.connect(dbname = dbname,
                                user = username, 
                                password = password,
            ...
            # Uncomment to see batching process
            #print("Inserting:", batches_files_path_list[0])

            # Add batch
            insert_data_batch_table(cur, table_name, batched_data_dir_name + "/" + batches_files_path_list[0], column_names_list)
            conn.commit()

            del batches_files_path_list[0]

            cur.close()
            conn.close()
        
        except (Exception, psycopg2.DatabaseError) as error:
            print("Batching errors list:\n", error)
            cur.close()
            if conn is not None:
                conn.close()
                conn = None

            # Time stop before connection
            time.sleep(2)
...
def drop_zno_db(table_name = "ZNODATA"):
    conn = None
...
if __name__ == "__main__":
    ...
    drop_zno_db() # comment to prevent dropping
